...
useEffect(() => {
  // Если экран шире 1024px → это ПК → редиректим на десктопную версию
  if (typeof window !== "undefined" && window.innerWidth > 1024) {
    const currentPath = window.location.pathname;
    const search = window.location.search;
    const desktopPath = currentPath.replace("/sheet/", "/sheet/desktop/");
    window.location.replace(desktopPath + search);
  }
}, []);
// ← КОНЕЦ БЛОКА
